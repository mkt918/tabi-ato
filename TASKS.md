...
## 修正（2026-09-22、スマホの勝手な拡大）
- iOS Safari は文字 16px 未満の入力欄にふれると自動拡大する → タッチ端末（`pointer: coarse`）では入力欄・選択欄・コメント欄をすべて 16px に（タイトルは 18px）。見た目の行送りは変えない
- ボタン・写真枠・リンクなどに `touch-action: manipulation` を付けてダブルタップ拡大を止める。ページ全体のピンチ拡大は残す（見づらいときに拡大できるように）
- プレビューの写真枠は `touch-action: none`（指でのドラッグ＝入替・見せ位置調整をスクロールと競合させない）
